...
function Loading({ display }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      setRandomColor3("#" + Math.floor(Math.random() * 16777215).toString(16));
    }, 500);
  });
  ...
  return (
    ...
    >
      <div className={clsx(styles.loader)}>
        <div ref={loadOne} className={clsx(styles.inner, styles.one)}></div>
        <div ref={loadTwo} className={clsx(styles.inner, styles.two)}></div>
        <div ref={loadThree} className={clsx(styles.inner, styles.three)}></div>
      </div>
    </div>
  );
}
